...
public class DefaultDrmSessionManager<T extends ExoMediaCrypto> implements DrmSessionManager<T>, DefaultDrmSession.ProvisioningManager<T> {
    ...
    public class MediaDrmEventListener implements ExoMediaDrm.OnEventListener<T> {
        ...
        public void onEvent(ExoMediaDrm<? extends T> exoMediaDrm, byte[] bArr, int i9, int i10, byte[] bArr2) {
            if (DefaultDrmSessionManager.this.mode == 0) {
                DefaultDrmSessionManager.this.mediaDrmHandler.obtainMessage(i9, bArr).sendToTarget();
            }
        }
    }
    ...
    public DefaultDrmSessionManager(UUID uuid, ExoMediaDrm<T> exoMediaDrm, MediaDrmCallback mediaDrmCallback, HashMap<String, String> map, Handler handler, EventListener eventListener) {
        this(uuid, exoMediaDrm, mediaDrmCallback, map, handler, eventListener, false, 3);
    }
    ...
    private static DrmInitData.SchemeData getSchemeData(DrmInitData drmInitData, UUID uuid, boolean z8) {
        ...
        while (true) {
            ...
            if (!schemeData.matches(uuid) && (!C3322C.CLEARKEY_UUID.equals(uuid) || !schemeData.matches(C3322C.COMMON_PSSH_UUID))) {
                z9 = false;
            }
            if (z9 && (schemeData.data != null || z8)) {
        ...
        if (C3322C.WIDEVINE_UUID.equals(uuid)) {
    ...
    private static String getSchemeMimeType(DrmInitData.SchemeData schemeData, UUID uuid) {
        ...
        return (Util.SDK_INT >= 26 || !C3322C.CLEARKEY_UUID.equals(uuid)) ? str : (MimeTypes.VIDEO_MP4.equals(str) || MimeTypes.AUDIO_MP4.equals(str)) ? "cenc" : str;
    }

    public static DefaultDrmSessionManager<FrameworkMediaCrypto> newFrameworkInstance(UUID uuid, MediaDrmCallback mediaDrmCallback, HashMap<String, String> map, Handler handler, EventListener eventListener) {
        return new DefaultDrmSessionManager<>(uuid, FrameworkMediaDrm.newInstance(uuid), mediaDrmCallback, map, handler, eventListener, false, 3);
    }
    ...
    public static DefaultDrmSessionManager<FrameworkMediaCrypto> newPlayReadyInstance(MediaDrmCallback mediaDrmCallback, String str, Handler handler, EventListener eventListener) {
        ...
        return newFrameworkInstance(C3322C.PLAYREADY_UUID, mediaDrmCallback, map, handler, eventListener);
    }

    public static DefaultDrmSessionManager<FrameworkMediaCrypto> newWidevineInstance(MediaDrmCallback mediaDrmCallback, HashMap<String, String> map, Handler handler, EventListener eventListener) {
        return newFrameworkInstance(C3322C.WIDEVINE_UUID, mediaDrmCallback, map, handler, eventListener);
    }
    ...
    public DrmSession<T> acquireSession(Looper looper, DrmInitData drmInitData) {
        ...
        if (defaultDrmSession == 0) {
            DefaultDrmSession<T> defaultDrmSession2 = new DefaultDrmSession<>(this.uuid, this.mediaDrm, this, bArr, schemeMimeType, this.mode, this.offlineLicenseKeySetId, this.optionalKeyRequestParameters, this.callback, looper, this.eventHandler, this.eventListener, this.initialDrmRequestRetryCount);
    ...
    public boolean canAcquireSession(DrmInitData drmInitData) {
        ...
        if (getSchemeData(drmInitData, this.uuid, true) == null) {
            if (drmInitData.schemeDataCount != 1 || !drmInitData.get(0).matches(C3322C.COMMON_PSSH_UUID)) {
                return false;
            }
        ...
        return !(C3322C.CENC_TYPE_cbc1.equals(str) || C3322C.CENC_TYPE_cbcs.equals(str) || C3322C.CENC_TYPE_cens.equals(str)) || Util.SDK_INT >= 24;
    }
    ...
    public DefaultDrmSessionManager(UUID uuid, ExoMediaDrm<T> exoMediaDrm, MediaDrmCallback mediaDrmCallback, HashMap<String, String> map, Handler handler, EventListener eventListener, boolean z8) {
        this(uuid, exoMediaDrm, mediaDrmCallback, map, handler, eventListener, z8, 3);
    }

    public DefaultDrmSessionManager(UUID uuid, ExoMediaDrm<T> exoMediaDrm, MediaDrmCallback mediaDrmCallback, HashMap<String, String> map, Handler handler, EventListener eventListener, boolean z8, int i9) {
        Assertions.checkNotNull(uuid);
        Assertions.checkNotNull(exoMediaDrm);
        Assertions.checkArgument(!C3322C.COMMON_PSSH_UUID.equals(uuid), "Use C.CLEARKEY_UUID instead");
        this.uuid = uuid;
