...
public class InterpolateOnScrollPositionChangeHelper {
    ...
    public void updateInterpolationForScreenPosition() {
        ...
        if (top < 0) {
            this.materialShapeDrawable.setInterpolation(Math.max(BitmapDescriptorFactory.HUE_RED, Math.min(1.0f, (top / height) + 1.0f)));
            this.shapedView.invalidate();
            return;
        }
        if (top + height > height2) {
            this.materialShapeDrawable.setInterpolation(Math.max(BitmapDescriptorFactory.HUE_RED, Math.min(1.0f, 1.0f - ((r0 - height2) / height))));
            this.shapedView.invalidate();
