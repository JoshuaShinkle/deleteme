...
public class JobInfoScheduler implements WorkScheduler {
    ...
    private boolean isJobServiceOn(JobScheduler jobScheduler, int i9, int i10) {
        for (JobInfo jobInfo : jobScheduler.getAllPendingJobs()) {
            int i11 = jobInfo.getExtras().getInt(ATTEMPT_NUMBER);
            if (jobInfo.getId() == i9) {
                return i11 >= i10;
            }
        }
    ...
    public void schedule(TransportContext transportContext, int i9) {
        schedule(transportContext, i9, false);
    }

    @Override // com.google.android.datatransport.runtime.scheduling.jobscheduling.WorkScheduler
    public void schedule(TransportContext transportContext, int i9, boolean z8) {
        ...
        if (!z8 && isJobServiceOn(jobScheduler, jobId, i9)) {
            Logging.m17455d(LOG_TAG, "Upload for context %s is already scheduled. Returning...", transportContext);
            return;
        }
        long nextCallTime = this.eventStore.getNextCallTime(transportContext);
        JobInfo.Builder builderConfigureJob = this.config.configureJob(new JobInfo.Builder(jobId, componentName), transportContext.getPriority(), nextCallTime, i9);
        PersistableBundle persistableBundle = new PersistableBundle();
        persistableBundle.putInt(ATTEMPT_NUMBER, i9);
        ...
        Logging.m17457d(LOG_TAG, "Scheduling upload for context %s with jobId=%d in %dms(Backend next call timestamp %d). Attempt %d", transportContext, Integer.valueOf(jobId), Long.valueOf(this.config.getScheduleDelay(transportContext.getPriority(), nextCallTime, i9)), Long.valueOf(nextCallTime), Integer.valueOf(i9));
