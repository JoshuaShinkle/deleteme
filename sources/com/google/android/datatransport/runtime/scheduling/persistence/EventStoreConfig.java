...
abstract class EventStoreConfig {
    ...
    static final EventStoreConfig DEFAULT = builder().setMaxStorageSizeInBytes(MAX_DB_STORAGE_SIZE_IN_BYTES).setLoadBatchSize(LOAD_BATCH_SIZE).setCriticalSectionEnterTimeoutMs(10000).setEventCleanUpAge(DURATION_ONE_WEEK_MS).setMaxBlobByteSizePerRow(MAX_BLOB_BYTE_SIZE_PER_ROW).build();

    @AutoValue.Builder
    public static abstract class Builder {
        public abstract EventStoreConfig build();

        public abstract Builder setCriticalSectionEnterTimeoutMs(int i9);

        public abstract Builder setEventCleanUpAge(long j9);

        public abstract Builder setLoadBatchSize(int i9);

        public abstract Builder setMaxBlobByteSizePerRow(int i9);

        public abstract Builder setMaxStorageSizeInBytes(long j9);
    }
