package com.google.zxing.client.android.result;

import android.app.Activity;
import com.google.zxing.Result;
import com.google.zxing.client.android.C4453R;
import com.google.zxing.client.result.ISBNParsedResult;
import com.google.zxing.client.result.ParsedResult;

/* loaded from: classes2.dex */
public final class ISBNResultHandler extends ResultHandler {
    private static final int[] buttons = {C4453R.string.button_product_search, C4453R.string.button_book_search, C4453R.string.button_search_book_contents, C4453R.string.button_custom_product_search};

    public ISBNResultHandler(Activity activity, ParsedResult parsedResult, Result result) {
        super(activity, parsedResult, result);
    }

    @Override // com.google.zxing.client.android.result.ResultHandler
    public int getButtonCount() {
        return hasCustomProductSearch() ? buttons.length : buttons.length - 1;
    }

    @Override // com.google.zxing.client.android.result.ResultHandler
    public int getButtonText(int i9) {
        return buttons[i9];
    }

    @Override // com.google.zxing.client.android.result.ResultHandler
    public int getDisplayTitle() {
        return C4453R.string.result_isbn;
    }

    @Override // com.google.zxing.client.android.result.ResultHandler
    public void handleButtonPress(int i9) {
        ISBNParsedResult iSBNParsedResult = (ISBNParsedResult) getResult();
        if (i9 == 0) {
            openProductSearch(iSBNParsedResult.getISBN());
            return;
        }
        if (i9 == 1) {
            openBookSearch(iSBNParsedResult.getISBN());
        } else if (i9 == 2) {
            searchBookContents(iSBNParsedResult.getISBN());
        } else {
            if (i9 != 3) {
                return;
            }
            openURL(fillInCustomSearchURL(iSBNParsedResult.getISBN()));
        }
    }
}
