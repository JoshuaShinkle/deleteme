...
class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            this.logger.m17776w("Failed to parse settings JSON from " + this.url, e9);
            this.logger.m17775w("Settings response " + str);
            return null;
        }
    }
    ...
    public JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        this.logger.m17773v("Settings response code was: " + iCode);
        if (requestWasSuccessful(iCode)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        this.logger.m17769e("Settings request failed; (status: " + iCode + ") from " + this.url);
        return null;
    }

    @Override // com.google.firebase.crashlytics.internal.settings.SettingsSpiCall
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z8) {
        if (!z8) {
        ...
        try {
            ...
            this.logger.m17767d("Requesting settings from " + this.url);
            this.logger.m17773v("Settings query params were: " + queryParamsFor);
            return handleResponse(httpGetRequestApplyHeadersTo.execute());
        } catch (IOException e9) {
            this.logger.m17770e("Settings request failed.", e9);
            return null;
        }
    }

    public boolean requestWasSuccessful(int i9) {
