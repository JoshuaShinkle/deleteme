...
public class CrashlyticsCore {
        ...
        try {
            this.breadcrumbSource.registerBreadcrumbHandler(new BreadcrumbHandler() { // from class: com.google.firebase.crashlytics.internal.common.c
                ...
                public final void handleBreadcrumb(String str) {
                    this.f15537a.log(str);
                }
            });
            this.controller.saveVersionControlInfo();
            if (!settingsProvider.getSettingsSync().featureFlagData.collectReports) {
                Logger.getLogger().m17767d("Collection of crash reports disabled in Crashlytics settings.");
                return Tasks.forException(new RuntimeException("Collection of crash reports disabled in Crashlytics settings."));
            }
            if (!this.controller.finalizeSessions(settingsProvider)) {
                Logger.getLogger().m17775w("Previous sessions could not be finalized.");
            }
            return this.controller.submitAllReports(settingsProvider.getSettingsAsync());
        } catch (Exception e9) {
            Logger.getLogger().m17770e("Crashlytics encountered a problem during asynchronous initialization.", e9);
    ...
    private void finishInitSynchronously(final SettingsProvider settingsProvider) throws ExecutionException, InterruptedException, TimeoutException {
        ...
        Logger.getLogger().m17767d("Crashlytics detected incomplete initialization on previous app launch. Will initialize synchronously.");
        try {
            futureSubmit.get(3L, TimeUnit.SECONDS);
        } catch (InterruptedException e9) {
            Logger.getLogger().m17770e("Crashlytics was interrupted during initialization.", e9);
        } catch (ExecutionException e10) {
            Logger.getLogger().m17770e("Crashlytics encountered a problem during initialization.", e10);
        } catch (TimeoutException e11) {
            Logger.getLogger().m17770e("Crashlytics timed out during initialization.", e11);
        }
    }
    ...
    public static boolean isBuildIdValid(String str, boolean z8) {
        if (!z8) {
            Logger.getLogger().m17773v("Configured not to require a build ID.");
            return true;
        }
    ...
    public void logFatalException(Throwable th) {
        Logger.getLogger().m17767d("Recorded on-demand fatal events: " + this.onDemandCounter.getRecordedOnDemandExceptions());
        Logger.getLogger().m17767d("Dropped on-demand fatal events: " + this.onDemandCounter.getDroppedOnDemandExceptions());
        ...
        this.backgroundWorker.submit(new Callable<Boolean>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsCore.3
            ...
            public Boolean call() {
                try {
                    ...
                    if (!zRemove) {
                        Logger.getLogger().m17775w("Initialization marker file was not properly removed.");
                    }
                    return Boolean.valueOf(zRemove);
                } catch (Exception e9) {
                    Logger.getLogger().m17770e("Problem encountered deleting Crashlytics initialization marker.", e9);
    ...
    public void markInitializationStarted() {
        ...
        Logger.getLogger().m17773v("Initialization marker file was created.");
    }
    ...
    public boolean onPreExecute(AppData appData, SettingsProvider settingsProvider) {
        ...
        try {
            ...
            if (!zDidPreviousInitializationFail || !CommonUtils.canTryConnection(this.context)) {
                Logger.getLogger().m17767d("Successfully configured exception handler.");
                return true;
            }
            Logger.getLogger().m17767d("Crashlytics did not finish previous background initialization. Initializing synchronously.");
            finishInitSynchronously(settingsProvider);
            return false;
        } catch (Exception e9) {
            Logger.getLogger().m17770e("Crashlytics was not started due to an exception during initialization", e9);
