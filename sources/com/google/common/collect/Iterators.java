...
public final class Iterators {
    ...
    public static final class ArrayItr<T> extends AbstractIndexedListIterator<T> {
        ...
        public ArrayItr(T[] tArr, int i9, int i10, int i11) {
            super(i10, i11);
            this.array = tArr;
            this.offset = i9;
        }
    ...
    @CanIgnoreReturnValue
    public static int advance(Iterator<?> it, int i9) {
        Preconditions.checkNotNull(it);
        int i10 = 0;
        Preconditions.checkArgument(i9 >= 0, "numberToAdvance must be nonnegative");
    ...
    public static void checkNonnegative(int i9) {
        if (i9 >= 0) {
            return;
        }
    ...
    public static <T> T get(Iterator<T> it, int i9) {
        checkNonnegative(i9);
        int iAdvance = advance(it, i9);
        if (it.hasNext()) {
            return it.next();
        }
        throw new IndexOutOfBoundsException("position (" + i9 + ") must be less than the number of elements that remained (" + iAdvance + ")");
    }
    ...
    public static <T> Iterator<T> limit(final Iterator<T> it, final int i9) {
        Preconditions.checkNotNull(it);
        Preconditions.checkArgument(i9 >= 0, "limit is negative");
        return new Iterator<T>() { // from class: com.google.common.collect.Iterators.7
            private int count;

            @Override // java.util.Iterator
            public boolean hasNext() {
                return this.count < i9 && it.hasNext();
            }
    ...
    public static <T> UnmodifiableIterator<List<T>> paddedPartition(Iterator<T> it, int i9) {
        return partitionImpl(it, i9, true);
    }

    public static <T> UnmodifiableIterator<List<T>> partition(Iterator<T> it, int i9) {
        return partitionImpl(it, i9, false);
    }

    private static <T> UnmodifiableIterator<List<T>> partitionImpl(final Iterator<T> it, final int i9, final boolean z8) {
        Preconditions.checkNotNull(it);
        ...
        return new UnmodifiableIterator<List<T>>() { // from class: com.google.common.collect.Iterators.4
            ...
            public List<T> next() {
                ...
                return (z8 || i10 == i9) ? listUnmodifiableList : listUnmodifiableList.subList(0, i10);
            }
        };
    }
    ...
    public static <T> UnmodifiableIterator<T> singletonIterator(final T t8) {
    ...
    public static <T> UnmodifiableListIterator<T> forArray(T[] tArr, int i9, int i10, int i11) {
        Preconditions.checkArgument(i10 >= 0);
        Preconditions.checkPositionIndexes(i9, i9 + i10, tArr.length);
        Preconditions.checkPositionIndex(i11, i10);
        return i10 == 0 ? emptyListIterator() : new ArrayItr(tArr, i9, i10, i11);
    }
    ...
    public static <T> T get(Iterator<? extends T> it, int i9, T t8) {
        checkNonnegative(i9);
        advance(it, i9);
    ...
    public static <T> T find(Iterator<? extends T> it, Predicate<? super T> predicate, T t8) {
        Preconditions.checkNotNull(it);
        Preconditions.checkNotNull(predicate);
        while (it.hasNext()) {
            T next = it.next();
