...
public final class TreeRangeMap<K extends Comparable, V> implements RangeMap<K, V> {
    ...
    public static final class RangeMapEntry<K extends Comparable, V> extends AbstractMapEntry<Range<K>, V> {
        ...
        public RangeMapEntry(Cut<K> cut, Cut<K> cut2, V v8) {
            this(Range.create(cut, cut2), v8);
        }

        public boolean contains(K k9) {
            return this.range.contains(k9);
        }
        ...
        public RangeMapEntry(Range<K> range, V v8) {
            this.range = range;
            this.value = v8;
        }
    ...
    public class SubRangeMap implements RangeMap<K, V> {
        ...
        public class SubRangeMapAsMap extends AbstractMap<Range<K>, V> {
            ...
            public Set<Map.Entry<Range<K>, V>> entrySet() {
                return new Maps.EntrySet<Range<K>, V>() { // from class: com.google.common.collect.TreeRangeMap.SubRangeMap.SubRangeMapAsMap.2
                    ...
                    public boolean retainAll(Collection<?> collection) {
                        return SubRangeMapAsMap.this.removeEntryIf(Predicates.not(Predicates.m17552in(collection)));
                    }
            ...
            public Set<Range<K>> keySet() {
                return new Maps.KeySet<Range<K>, V>(this) { // from class: com.google.common.collect.TreeRangeMap.SubRangeMap.SubRangeMapAsMap.1
                    ...
                    public boolean retainAll(Collection<?> collection) {
                        return SubRangeMapAsMap.this.removeEntryIf(Predicates.compose(Predicates.not(Predicates.m17552in(collection)), Maps.keyFunction()));
                    }
                };
            }
            ...
            public Collection<V> values() {
                return new Maps.Values<Range<K>, V>(this) { // from class: com.google.common.collect.TreeRangeMap.SubRangeMap.SubRangeMapAsMap.4
                    ...
                    public boolean removeAll(Collection<?> collection) {
                        return SubRangeMapAsMap.this.removeEntryIf(Predicates.compose(Predicates.m17552in(collection), Maps.valueFunction()));
                    }

                    @Override // com.google.common.collect.Maps.Values, java.util.AbstractCollection, java.util.Collection
                    public boolean retainAll(Collection<?> collection) {
                        return SubRangeMapAsMap.this.removeEntryIf(Predicates.compose(Predicates.not(Predicates.m17552in(collection)), Maps.valueFunction()));
                    }
                };
            }
        }
        ...
        @Override // com.google.common.collect.RangeMap
        public V get(K k9) {
            if (this.subRange.contains(k9)) {
                return (V) TreeRangeMap.this.get(k9);
            }
            return null;
        }
        ...
        public Map.Entry<Range<K>, V> getEntry(K k9) {
            ...
            if (!this.subRange.contains(k9) || (entry = TreeRangeMap.this.getEntry(k9)) == null) {
                return null;
            }
        ...
        public void putCoalescing(Range<K> range, V v8) {
            if (TreeRangeMap.this.entriesByLowerBound.isEmpty() || range.isEmpty() || !this.subRange.encloses(range)) {
                ...
                put(TreeRangeMap.this.coalescedRange(range, Preconditions.checkNotNull(v8)).intersection(this.subRange), v8);
            }
        }
    ...
    private static <K extends Comparable, V> Range<K> coalesce(Range<K> range, V v8, Map.Entry<Cut<K>, RangeMapEntry<K, V>> entry) {
        return (entry != null && entry.getValue().getKey().isConnected(range) && entry.getValue().getValue().equals(v8)) ? (Range<K>) range.span(entry.getValue().getKey()) : range;
    }

    /* JADX INFO: Access modifiers changed from: private */
    public Range<K> coalescedRange(Range<K> range, V v8) {
        return coalesce(coalesce(range, v8, this.entriesByLowerBound.lowerEntry(range.lowerBound)), v8, this.entriesByLowerBound.floorEntry(range.upperBound));
    }
    ...
    private void putRangeMapEntry(Cut<K> cut, Cut<K> cut2, V v8) {
        this.entriesByLowerBound.put(cut, new RangeMapEntry<>(cut, cut2, v8));
    }
    ...
    @Override // com.google.common.collect.RangeMap
    public Map.Entry<Range<K>, V> getEntry(K k9) {
        Map.Entry<Cut<K>, RangeMapEntry<K, V>> entryFloorEntry = this.entriesByLowerBound.floorEntry(Cut.belowValue(k9));
        if (entryFloorEntry == null || !entryFloorEntry.getValue().contains(k9)) {
            return null;
        }
    ...
    public void put(Range<K> range, V v8) {
        ...
        remove(range);
        this.entriesByLowerBound.put(range.lowerBound, new RangeMapEntry(range, v8));
    }
    ...
    public void putCoalescing(Range<K> range, V v8) {
        if (this.entriesByLowerBound.isEmpty()) {
            ...
            put(coalescedRange(range, Preconditions.checkNotNull(v8)), v8);
        }
    }
