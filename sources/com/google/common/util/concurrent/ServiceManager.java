...
public final class ServiceManager {
    ...
    public ServiceManager(Iterable<? extends Service> iterable) {
        ...
        if (immutableListCopyOf.isEmpty()) {
            ...
            immutableListCopyOf = ImmutableList.m17574of(new NoOpService());
        }
    ...
    public ServiceManager startAsync() {
        ...
        while (it2.hasNext()) {
            ...
            try {
                ...
                logger.log(Level.WARNING, "Unable to start Service " + next2, (Throwable) e9);
            }
        }
        return this;
    }
    ...
    public static final class ServiceManagerState {
        ...
        public void awaitStopped(long j9, TimeUnit timeUnit) {
            this.monitor.enter();
            try {
                if (this.monitor.waitForUninterruptibly(this.stoppedGuard, j9, timeUnit)) {
                    return;
                }
                throw new TimeoutException("Timeout waiting for the services to stop. The following services have not stopped: " + Multimaps.filterKeys((SetMultimap) this.servicesByState, Predicates.not(Predicates.m17552in(EnumSet.of(Service.State.TERMINATED, Service.State.FAILED)))));
            } finally {
                this.monitor.leave();
            }
        }

        public void awaitHealthy(long j9, TimeUnit timeUnit) {
            this.monitor.enter();
            try {
                if (this.monitor.waitForUninterruptibly(this.awaitHealthGuard, j9, timeUnit)) {
                    checkHealthy();
                    return;
                }
                throw new TimeoutException("Timeout waiting for the services to become healthy. The following services have not started: " + Multimaps.filterKeys((SetMultimap) this.servicesByState, Predicates.m17552in(ImmutableSet.m17617of(Service.State.NEW, Service.State.STARTING))));
            } finally {
                this.monitor.leave();
            }
        }
    ...
    public void awaitHealthy(long j9, TimeUnit timeUnit) {
        this.state.awaitHealthy(j9, timeUnit);
    }

    public void awaitStopped(long j9, TimeUnit timeUnit) {
        this.state.awaitStopped(j9, timeUnit);
    }
}
