...
public final class ImmutableValueGraph<N, V> extends ConfigurableValueGraph<N, V> {
    ...
    private static <N, V> GraphConnections<N, V> connectionsOf(final ValueGraph<N, V> valueGraph, final N n8) {
        Function<N, V> function = new Function<N, V>() { // from class: com.google.common.graph.ImmutableValueGraph.1
            /* JADX WARN: Multi-variable type inference failed */
            @Override // com.google.common.base.Function
            public V apply(N n9) {
                return (V) valueGraph.edgeValueOrDefault(n8, n9, null);
            }
        };
        return valueGraph.isDirected() ? DirectedGraphConnections.ofImmutable(valueGraph.predecessors((ValueGraph<N, V>) n8), Maps.asMap(valueGraph.successors((ValueGraph<N, V>) n8), function)) : UndirectedGraphConnections.ofImmutable(Maps.asMap(valueGraph.adjacentNodes(n8), function));
    }
    ...
    private static <N, V> ImmutableMap<N, GraphConnections<N, V>> getNodeConnections(ValueGraph<N, V> valueGraph) {
        ...
        for (N n8 : valueGraph.nodes()) {
            builder.put(n8, connectionsOf(valueGraph, n8));
        }
