...
public class ImmutableGraph<N> extends ForwardingGraph<N> {
    ...
    private static <N> GraphConnections<N, GraphConstants.Presence> connectionsOf(Graph<N> graph, N n8) {
        Function functionConstant = Functions.constant(GraphConstants.Presence.EDGE_EXISTS);
        return graph.isDirected() ? DirectedGraphConnections.ofImmutable(graph.predecessors((Graph<N>) n8), Maps.asMap(graph.successors((Graph<N>) n8), functionConstant)) : UndirectedGraphConnections.ofImmutable(Maps.asMap(graph.adjacentNodes(n8), functionConstant));
    }
    ...
    private static <N> ImmutableMap<N, GraphConnections<N, GraphConstants.Presence>> getNodeConnections(Graph<N> graph) {
        ...
        for (N n8 : graph.nodes()) {
            builder.put(n8, connectionsOf(graph, n8));
        }
