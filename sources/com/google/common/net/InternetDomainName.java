...
public final class InternetDomainName {
    ...
    private static final Splitter DOT_SPLITTER = Splitter.m17556on(ClassUtils.PACKAGE_SEPARATOR_CHAR);
    private static final Joiner DOT_JOINER = Joiner.m17549on(ClassUtils.PACKAGE_SEPARATOR_CHAR);

    static {
        CharMatcher charMatcherAnyOf = CharMatcher.anyOf("-_");
        DASH_MATCHER = charMatcherAnyOf;
        PART_CHAR_MATCHER = CharMatcher.javaLetterOrDigit().mo17548or(charMatcherAnyOf);
    }
    ...
    private InternetDomainName ancestor(int i9) {
        Joiner joiner = DOT_JOINER;
        ImmutableList<String> immutableList = this.parts;
        return from(joiner.join(immutableList.subList(i9, immutableList.size())));
    }

    private int findPublicSuffix() {
        int size = this.parts.size();
        for (int i9 = 0; i9 < size; i9++) {
            String strJoin = DOT_JOINER.join(this.parts.subList(i9, size));
    ...
    private static boolean validatePart(String str, boolean z8) {
        if (str.length() >= 1 && str.length() <= 63) {
            ...
            if (!charMatcher.matches(str.charAt(0)) && !charMatcher.matches(str.charAt(str.length() - 1))) {
                return (z8 && CharMatcher.digit().matches(str.charAt(0))) ? false : true;
            }
        }
        return false;
    }
    ...
    private static boolean validateSyntax(List<String> list) {
        ...
        for (int i9 = 0; i9 < size; i9++) {
            if (!validatePart(list.get(i9), false)) {
                return false;
            }
        }
        return true;
