...
public class EnumUtils {
    ...
    public static <E extends Enum<E>> long generateBitVector(Class<E> cls, Iterable<? extends E> iterable) {
        ...
        long jOrdinal = 0;
        while (it.hasNext()) {
            Validate.isTrue(it.next() != null, NULL_ELEMENTS_NOT_PERMITTED, new Object[0]);
            jOrdinal |= 1 << r7.ordinal();
        }
        return jOrdinal;
    }
    ...
    public static <E extends Enum<E>> EnumSet<E> processBitVector(Class<E> cls, long j9) {
        checkBitVectorable(cls).getEnumConstants();
        return processBitVectors(cls, j9);
    }
    ...
    public static <E extends Enum<E>> EnumSet<E> processBitVectors(Class<E> cls, long... jArr) {
        ...
        for (E e9 : cls.getEnumConstants()) {
            int iOrdinal = e9.ordinal() / 64;
            if (iOrdinal < jArrClone.length && (jArrClone[iOrdinal] & (1 << (e9.ordinal() % 64))) != 0) {
                enumSetNoneOf.add(e9);
            }
        }
    ...
    public static <E extends Enum<E>> long[] generateBitVectors(Class<E> cls, E... eArr) {
        ...
        while (it.hasNext()) {
            int iOrdinal = ((Enum) it.next()).ordinal() / 64;
            jArr[iOrdinal] = jArr[iOrdinal] | (1 << (r1.ordinal() % 64));
        }
        ArrayUtils.reverse(jArr);
        return jArr;
    }
